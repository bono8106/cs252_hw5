...
object Main {
  ...
  val actorSystem = ActorSystem("rocket")
	...
	// Immutable HashMap is 20-30% slower
	type Result = immutable.HashMap[String, TreeMap[String, Int]]
	type MResult = mutable.HashMap[String, mutable.Map[String, Int]]
	...
	def processToken(result: MResult, token : String, f: String, n: Int) {
		val fileMap = result.getOrElseUpdate(token, new mutable.HashMap[String, Int])
	...
	class MergeDispatchActor(parent: Promise[MResult]) extends Actor {
	  val fileRouter = context.actorOf(Props[FileActor].withRouter(RoundRobinRouter(nrOfInstances = 8)))
  ...
  def main(args: Array[String]) {
    ...
    if (dump) {
    	val sorted = new TreeMap[String, TreeMap[String, Int]] ++ result.map(_ match { case (keyword, files) => (keyword, new TreeMap[String, Int] ++ files) })
